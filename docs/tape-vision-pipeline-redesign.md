...
現行版本保留上一筆已接受的有序中心線。穩定追蹤時，
`TemporalCenterlineTracker` 沿上一幀各取樣點的局部法線掃描目前候選遮罩，
選擇距離最近且寬度相容的連續黑色區段中點，形成新的稀疏中心線。
這條快速路徑只更新已有路徑，不負責取得、分支或閉環判定。

下列情況直接或立即回到完整 `CenterlineExtractor`：

- 尚無上一幀中心線、路徑曾有分支或閉環。
- 支持切片不足、連續缺失過多、點距／寬度／位移超限。
- 快速結果不足以維持 `FULL_PATH`，或無法解釋至少 70% 候選面積。
- 每三個快速影格後的週期性完整驗證。

快速結果仍必須通過紙板色彩、地板脈絡、路徑長度、曲率及上一幀連續性等既有
候選閘門；它不能因為符合歷史位置而取得控制權。
